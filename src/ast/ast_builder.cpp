...
std::unique_ptr<ASTNode> ASTBuilder::VisitNode(LTLParser::FormulaContext* ctx) {
  ...
  } else if (auto* eventually_ctx =
                 dynamic_cast<LTLParser::EventuallyContext*>(ctx)) {
  ...
  } else if (auto* implies_ctx =
                 dynamic_cast<LTLParser::ImpliesContext*>(ctx)) {
...
std::unique_ptr<ASTNode> ASTBuilder::VisitAlways(
...
std::unique_ptr<ASTNode> ASTBuilder::VisitEventually(
    LTLParser::EventuallyContext* ctx) {
  return std::make_unique<UntilASTNode>(std::make_unique<TrueASTNode>(),
                                        VisitNode(ctx->formula()));
}

std::unique_ptr<ASTNode> ASTBuilder::VisitAnd(LTLParser::AndContext* ctx) {
  return std::make_unique<AndASTNode>(VisitNode(ctx->formula(0)),
                                      VisitNode(ctx->formula(1)));
}
...
std::unique_ptr<ASTNode> ASTBuilder::VisitImplies(
    LTLParser::ImpliesContext* ctx) {
  auto left = VisitNode(ctx->formula(0));
  auto right = VisitNode(ctx->formula(1));
  if (right) {
    right->Negate();
...
std::unique_ptr<ASTNode> ASTBuilder::VisitUntil(LTLParser::UntilContext* ctx) {
  return std::make_unique<UntilASTNode>(VisitNode(ctx->formula(0)),
                                        VisitNode(ctx->formula(1)));
}

std::unique_ptr<ASTNode> ASTBuilder::VisitTrueLit(
    LTLParser::TrueLitContext* ctx) {
...
std::unique_ptr<ASTNode> ASTBuilder::VisitAtomExpr(
    LTLParser::AtomExprContext* ctx) {

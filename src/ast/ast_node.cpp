...
std::string ASTNode::NegatedString() const { return is_negated_ ? " [Negated]" : ""; }
...
AndASTNode::AndASTNode(std::unique_ptr<ASTNode> left, std::unique_ptr<ASTNode> right,
                       bool is_negated)
...
UntilASTNode::UntilASTNode(std::unique_ptr<ASTNode> left, std::unique_ptr<ASTNode> right,
                           bool is_negated)
    : ASTNode(is_negated), left_(std::move(left)), right_(std::move(right)) {
  left_->set_parent(this);
  right_->set_parent(this);
